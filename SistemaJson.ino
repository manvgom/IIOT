...
void loop() {
  ...
  float t_camion = dhtSensor.readTemperature();
  float lat_camion = 42.05606;
  float lon_camion = 2.05234;
  float h_fabrica = dhtSensor.readHumidity();           
  float t_fabrica = dhtSensor.readTemperature();
  float h_granja = dhtSensor.readHumidity(); 
  float t_granja = dhtSensor.readTemperature();
  float tc_granja = dhtSensor.readTemperature();
  float h_oficina = dhtSensor.readHumidity();
  float t_oficina = dhtSensor.readTemperature();
  float t_vaca = dhtSensor.readTemperature();
  float lat_vaca = 42.606;
  float lon_vaca = 2.234;
  float comida = 100;  //scale.get_units(10);
  float peso = 540;   //scale.get_units(10);
